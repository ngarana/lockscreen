...
WAYLAND_EXPORT void mock_pam_force_rc(int rc) { g_pam_forced_rc = rc; }
WAYLAND_EXPORT const char *mock_pam_last_user() { return g_pam_user; }
...
WAYLAND_EXPORT int pam_authenticate(pam_handle_t *pamh, int flags) {
    ...
    bool ok = resp && resp[1].resp &&
              std::strcmp(resp[1].resp, g_pam_expected_password) == 0;
    if (resp) {
...
WAYLAND_EXPORT int mpv_set_property(mpv_handle *ctx, const char *name, mpv_format format, void *data) {
    return 0;
}

WAYLAND_EXPORT int mpv_get_property(mpv_handle *ctx, const char *name, mpv_format format, void *data) {
...
WAYLAND_EXPORT int mpv_render_context_create(mpv_render_context **res, mpv_handle *mpv, mpv_render_param *params) {
    if (res) {
        *res = (mpv_render_context*)0x3333;
    }
    return 0;
}
...
WAYLAND_EXPORT int wl_display_roundtrip(struct wl_display *display) {
    ...
    if (roundtrip_count == 1 && g_registry_listener && g_registry_data) {
        typedef void (*global_cb_t)(void *data, void *registry, uint32_t name, const char *interface, uint32_t version);
...
WAYLAND_EXPORT int wl_proxy_add_listener(struct wl_proxy *proxy, void (**implementation)(void), void *data) {
...
WAYLAND_EXPORT struct wl_cursor_theme *wl_cursor_theme_load(const char *name, int size, struct wl_shm *shm) {
    return (struct wl_cursor_theme*)0x6666;
}

WAYLAND_EXPORT struct wl_cursor *wl_cursor_theme_get_cursor(struct wl_cursor_theme *theme, const char *name) {
...
WAYLAND_EXPORT void mock_xkb_set_repeats(int r) { g_xkb_repeats = r; }
...
WAYLAND_EXPORT struct xkb_keymap *xkb_keymap_new_from_string(struct xkb_context *context, const char *string,
...
WAYLAND_EXPORT int xkb_state_key_get_utf8(struct xkb_state *state, uint32_t key, char *buffer, size_t size) {
...
WAYLAND_EXPORT int xkb_state_mod_name_is_active(struct xkb_state *state, const char *name, int type) {
    return 0;
}
...
WAYLAND_EXPORT int sd_bus_call_method(struct sd_bus *bus, const char *destination, const char *path,
                                     const char *interface, const char *member, struct sd_bus_error *ret_error,
...
WAYLAND_EXPORT int sd_bus_add_filter(struct sd_bus *bus, struct sd_bus_slot **slot, int (*callback)(struct sd_bus_message *m, void *userdata, struct sd_bus_error *ret_error), void *userdata) {
    return 0;
}
...
WAYLAND_EXPORT int sd_bus_message_is_method_call(struct sd_bus_message *m, const char *interface, const char *member) {
    return 0;
}
...
WAYLAND_EXPORT int sd_bus_message_enter_container(struct sd_bus_message *m, char type, const char *contents) {
    return 0;
}
...
WAYLAND_EXPORT int sd_bus_message_is_signal(struct sd_bus_message *m, const char *interface, const char *member) {
    return 0;
}

WAYLAND_EXPORT int sd_bus_message_peek_type(struct sd_bus_message *m, char *type, const char **contents) {
    return -1;
}
...
WAYLAND_EXPORT int sd_bus_add_object_vtable(struct sd_bus *bus, struct sd_bus_slot **slot, const char *path, const char *interface, const sd_bus_vtable *vtable, void *userdata) {
    return 0;
}
...
WAYLAND_EXPORT int sd_bus_message_new_method_return(struct sd_bus_message *call, struct sd_bus_message **reply) {
    return -1;
}

WAYLAND_EXPORT int sd_bus_message_open_container(struct sd_bus_message *m, char type, const char *contents) {
    return 0;
}
