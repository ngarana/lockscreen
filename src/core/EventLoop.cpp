...
void EventLoop::removeFd(int fd) {
    if (fds_.erase(fd)) epoll_ctl(epollFd_, EPOLL_CTL_DEL, fd, nullptr);
}
...
int EventLoop::addTimer(int64_t intervalMs, bool repeat, Callback cb) {
    ...
    timers_[tfd] = Timer{std::move(cb), repeat};
...
void EventLoop::post(Callback cb) {
    {
        std::lock_guard<std::mutex> lk(postMutex_);
...
void EventLoop::quit() {
    running_ = false;
    uint64_t one = 1;
    ssize_t r = write(wakeFd_, &one, sizeof(one));  // wake epoll_wait
    (void)r;
}
