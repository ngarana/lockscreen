...
struct Color {
    ...
    static Color fromHex(const std::string& hex) {
        ...
        if (h.size() == 8) {          // AARRGGBB
            c.a = byte(0); c.r = byte(2); c.g = byte(4); c.b = byte(6);
        } else if (h.size() == 6) {   // RRGGBB
            c.r = byte(0); c.g = byte(2); c.b = byte(4); c.a = 1.0;
        }
        return c;
    }

    Color withAlpha(double alpha) const { Color c = *this; c.a = alpha; return c; }
...
public:
    ...
    void set(double v) { from_ = to_ = v; start_ = 0; }
    ...
    bool active(int64_t now) const {
        return duration_ > 0 && (now - start_) < duration_;
    }
