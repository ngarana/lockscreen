...
void readProp(sd_bus_message* m, const char* key, MenuNode& n) {
    if (std::strcmp(key, "label") == 0) {
        ...
        const char* s = nullptr;
        if (sd_bus_message_enter_container(m, 'v', "s") >= 0) {
            if (sd_bus_message_read_basic(m, 's', &s) >= 0 && s) n.separator = std::strcmp(s, "separator") == 0;
            sd_bus_message_exit_container(m);
        } else {
            sd_bus_message_skip(m, "v");
        }
        ...
        const char* s = nullptr;
        if (sd_bus_message_enter_container(m, 'v', "s") >= 0) {
            if (sd_bus_message_read_basic(m, 's', &s) >= 0 && s) n.hasSubmenu = std::strcmp(s, "submenu") == 0;
            sd_bus_message_exit_container(m);
        } else {
            sd_bus_message_skip(m, "v");
        }
...
void DbusMenuBackend::clicked(const std::string& service, const std::string& menuPath,
                              int32_t id) {
    sd_bus* bus = bus_.get();
