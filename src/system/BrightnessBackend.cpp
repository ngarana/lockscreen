...
BrightnessBackend::BrightnessBackend(EventLoop& loop, SystemBus& bus)
    : loop_(loop), bus_(bus) {}
...
bool BrightnessBackend::start() {
    ...
    DIR* dir = opendir(kBacklightDir);
    if (!dir) return false;
    int bestMax = 0;
    while (dirent* e = readdir(dir)) {
        if (e->d_name[0] == '.') continue;
        std::string base = std::string(kBacklightDir) + "/" + e->d_name;
        int max = readIntFile(base + "/max_brightness");
        if (max > bestMax) {
            bestMax = max;
            snap_.device = e->d_name;
        }
    }
    closedir(dir);
...
bool BrightnessBackend::readSysfs() {
    ...
    int cur = readIntFile(base + "/brightness");
    int max = readIntFile(base + "/max_brightness");
    if (cur < 0 || max <= 0) {
...
void BrightnessBackend::onUdevEvent() {
    ...
    if (!sawEvent) return;
    if (readSysfs() && onChange_) onChange_();
}
...
void BrightnessBackend::setFraction(double frac) {
    ...
    int raw = std::max(1, static_cast<int>(frac * snap_.max + 0.5));
