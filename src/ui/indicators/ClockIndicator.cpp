...
public:
    ...
    void draw(Painter& p, int64_t now) override {
        ...
        if (prev_.contains(hoverX_, hoverY_)) p.fillRoundedRect(prev_, 8.0, theme::color::glassHover);
        if (next_.contains(hoverX_, hoverY_)) p.fillRoundedRect(next_, 8.0, theme::color::glassHover);
        ...
        for (int r = 0; r < kRows; ++r) {
            ...
            for (int c = 0; c < 7; ++c) {
                ...
                const bool isToday = inMonth && cell.tm_mday == todayD && dy == todayY && dm == todayM;
                const double cx = gridX + kWeekColW + c * kCellW + kCellW / 2.0;
        ...
        if (!zones_.empty()) {
            ...
            p.fillRectSource({b.x + kPad, ty, b.w - kPad * 2, 1.0}, theme::statusbar::panelSurfaceHover());
            ty += 6.0;
            for (const auto& z : zones_) {
                TextStyle ls{theme::font::family, 12.0, PANGO_WEIGHT_NORMAL, theme::color::text};
                TextStyle rs{theme::font::family, 12.0, PANGO_WEIGHT_BOLD, theme::color::textSubtle};
    ...
    bool handleClick(double x, double y) override {
        if (prev_.contains(x, y)) { --monthOffset_; return true; }
        if (next_.contains(x, y)) { ++monthOffset_; return true; }
        if (today_.contains(x, y)) { monthOffset_ = 0; return true; }
        return false;
    }
