...
SystemMonitorIndicator::SystemMonitorIndicator(const SystemBackends& backends)
    ...
    for (const auto& m : metrics) {
        if (m == "cpu") segs_.push_back({Metric::Cpu, kCpuGlyph, 0.0});
        else if (m == "mem" || m == "ram") segs_.push_back({Metric::Mem, kMemGlyph, 0.0});
    }
...
void SystemMonitorIndicator::poll(int64_t now) {
    ...
    if (!s.valid) return;
    for (auto& seg : segs_) {
        seg.value = seg.metric == Metric::Cpu ? s.cpuPercent : s.memPercent;
    }
}
...
double SystemMonitorIndicator::measureWidth(Painter& p) {
    ...
    for (size_t i = 0; i < segs_.size(); ++i) {
        w += p.measureText(segs_[i].glyph, icon).w + kGap + p.measureText(pctText(segs_[i].value), txt).w;
...
void SystemMonitorIndicator::draw(Painter& p, int64_t now) {
    ...
    if (alpha > 0.01) {
        ...
        r.y += 2.0;
        r.h -= 4.0;
        p.fillRoundedRect(r, 8.0, theme::color::glassHover.withAlpha(alpha * theme::color::glassHover.a));
    }
    if (focused) {
        Rect r = bounds;
        r.y += 1.0;
