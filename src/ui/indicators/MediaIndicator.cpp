...
public:
    ...
    void draw(Painter& p, int64_t now) override {
        ...
        buttons_.clear();
        struct Btn { const char* glyph; double dx; bool enabled; };
    ...
    bool handleClick(double x, double y) override {
        ...
        if (buttons_[0].contains(x, y)) { mpris_->previous(); return true; }
        if (buttons_[1].contains(x, y)) { mpris_->togglePlaying(); return true; }
        if (buttons_[2].contains(x, y)) { mpris_->next(); return true; }
        return false;
    }
...
MediaIndicator::MediaIndicator(const SystemBackends& backends)
    : StatusIndicator("media", Zone::Right, 50), mpris_(backends.mpris) {
    visible = false;  // hidden until something is actually playing
}

bool MediaIndicator::playing() const { return mpris_ && mpris_->playing(); }
