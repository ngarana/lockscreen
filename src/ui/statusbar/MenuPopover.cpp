...
MenuPopover::MenuPopover(DbusMenuBackend* backend, std::string service, std::string menuPath,
    ...
    : backend_(backend), service_(std::move(service)), menuPath_(std::move(menuPath)) {
    stack_.push_back({0, std::move(title), std::move(root)});
}

double MenuPopover::contentWidth() const { return kMenuW; }
...
void MenuPopover::draw(Painter& p, int64_t now) {
    ...
    if (stack_.size() > 1) {
        ...
        p.fillRect({b.x + kPad, y + kHeaderH - 1.0, b.w - kPad * 2, 1.0}, theme::color::glassBorder);
        y += kHeaderH;
    }
    ...
    for (const auto& it : cur().items) {
        ...
        p.drawText(row.x + kLeftCol, y + (kRowH - 14.0) / 2.0, it.label, ls, HAlign::Left, labelMax);
