...
std::string Clock::timeString() const { return formatNow("%H:%M"); }
std::string Clock::dateString() const { return formatNow("%A, %B %-d"); }
...
void Clock::draw(Painter& p, double centerX, double topY) const {
    ...
    p.drawTextShadowed(centerX, dateY, dateString(), dateStyle(), HAlign::Center, shadowA, shadowOff);
}
