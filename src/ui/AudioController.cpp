...
AudioController::AudioController(MprisController& mpris) : mpris_(mpris) {
    ...
    next_.label = "Next";
    prev_.onClick = [this] { mpris_.previous(); };
    playPause_.onClick = [this] { mpris_.togglePlaying(); };
    next_.onClick = [this] { mpris_.next(); };
}

bool AudioController::active() const { return mpris_.active(); }
void AudioController::refresh() { mpris_.refresh(); }
...
void AudioController::updateHover(double x, double y, int64_t now) {
    for (ActionButton* b : {&prev_, &playPause_, &next_})
        b->setHovered(b->contains(x, y), now);
}
