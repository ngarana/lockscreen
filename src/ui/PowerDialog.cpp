...
void PowerDialog::show(const std::string& icon, const std::string& title,
    ...
    barAnim_.set(1.0);
    if (autoConfirmMs > 0)
        barAnim_.animateTo(0.0, autoConfirmMs, ease::linear);
...
void PowerDialog::draw(Painter& p, int w, int h, int64_t now) {
    ...
    // ── Header: icon + title on one line ─────────────────────────────────
    {
        ...
        TextStyle is{theme::font::iconFamily, iconSz, PANGO_WEIGHT_NORMAL,
                     theme::color::primary};
        Size iconSzM = p.measureText(icon_, is);
        p.drawText(card.x + kPad, lineY, icon_, is, HAlign::Left);

        TextStyle ts{theme::font::family, 13.0, PANGO_WEIGHT_SEMIBOLD,
                     theme::color::text};
        double titleX = card.x + kPad + iconSzM.w + 7.0;
        p.drawText(titleX, lineY, title_, ts, HAlign::Left,
                   card.w - kPad * 2.0 - iconSzM.w - 7.0);
    }
    ...
    auto drawBtn = [&](const Rect& r, bool hovered, double scale,
                       const std::string& label, bool isPrimary) {
        const double sx = r.cx(), sy = r.cy();
        Rect sr{sx - r.w * scale / 2.0, sy - r.h * scale / 2.0,
                r.w * scale, r.h * scale};
        ...
        p.drawText(sr.cx() - lsz.w / 2.0, sr.cy() - lsz.h / 2.0,
                   label, ls, HAlign::Left);
    };
