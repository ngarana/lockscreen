...
double NotificationView::cardHeight(const Notification& n, Painter& p, double w) const {
    ...
    const std::string head = n.title.empty() ? n.app : n.title;
    const double titleH = p.measureText(head, titleStyle(), textW).h;
    const double bodyH = p.measureText(n.body, bodyStyle(), textW).h;
    const double textH = titleH + theme::spacing::small + bodyH;
...
void NotificationView::drawCard(Card& c, Painter& p, int64_t now, const Rect& r) {
    ...
    const std::string head = c.note.title.empty() ? c.note.app : c.note.title;
    ...
    auto paint = [&] {
        ...
        // Text block, vertically centred against the tile.
        const Size ts = p.measureText(head, titleStyle(), textW);
        const Size bs = p.measureText(c.note.body, bodyStyle(), textW);
        const double textH = ts.h + theme::spacing::small + bs.h;
        double ty = r.y + pad + std::max(0.0, (icon - textH) / 2.0);
        const double tx = r.x + pad + icon + pad;
        p.drawText(tx, ty, head, titleStyle(), HAlign::Left, textW);
        p.drawText(tx, ty + ts.h + theme::spacing::small, c.note.body, bodyStyle(),
                   HAlign::Left, textW);
    };
...
void NotificationView::draw(Painter& p, int64_t now, double left, double bottom,
    ...
    for (size_t i = start; i < cards_.size(); ++i)
        heights.push_back(cardHeight(cards_[i].note, p, w));
...
bool NotificationView::handlePress(double x, double y, int64_t) {
...
void NotificationView::updateHover(double x, double y, int64_t) {
    for (const auto& [idx, r] : layout_)
        if (idx < cards_.size()) cards_[idx].hovered = r.contains(x, y);
}
...
bool NotificationView::animating(int64_t now) const {
    for (const auto& c : cards_)
        if (c.appear.active(now)) return true;
    return false;
}
...
std::vector<Notification> demoNotifications() {
    return {
        ...
        {2, 0, "Mail", "New message from Priya",
         "Re: Q3 roadmap — please review the attached draft", "", theme::color::green},
