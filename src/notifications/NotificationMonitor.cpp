...
const Color kAccents[] = {
    theme::color::blue,  theme::color::green, theme::color::mauve,
    theme::color::peach, theme::color::teal,  theme::color::sky,
    theme::color::pink,  theme::color::yellow,
};
...
bool NotificationMonitor::start(bool seedFromLog) {
    ...
    int r = sd_bus_call_method(
        ...
        "org.freedesktop.DBus.Monitoring", "BecomeMonitor", &err, nullptr, "asu", 3u,
...
void NotificationMonitor::fetchBacklog(sd_bus* bus) {
    ...
    if (sd_bus_call_method(bus, notiflog::kBusName, notiflog::kObjectPath,
                           notiflog::kInterface, "List", &err, &reply, "") < 0) {
        std::fprintf(stderr, "qypr-lock: no notification log service (%s); "
...
void NotificationMonitor::teardown() {
    if (fd_ >= 0) { loop_.removeFd(fd_); fd_ = -1; }
    if (filter_) { sd_bus_slot_unref(filter_); filter_ = nullptr; }
    if (bus_) { sd_bus_unref(bus_); bus_ = nullptr; }
}

int NotificationMonitor::onMessage(sd_bus_message* m, void* userdata, sd_bus_error*) {
    auto* self = static_cast<NotificationMonitor*>(userdata);
    uint8_t type = 0;
    sd_bus_message_get_type(m, &type);
    if (type == SD_BUS_MESSAGE_METHOD_CALL &&
        sd_bus_message_is_method_call(m, "org.freedesktop.Notifications", "Notify"))
        self->handleNotify(m);
...
void NotificationMonitor::handleNotify(sd_bus_message* m) {
    ...
    if (transient) return;  // volume OSDs and the like: never queued
    if ((!summary || !*summary) && (!body || !*body)) return;
...
void NotificationMonitor::handleReturn(sd_bus_message* m) {
    ...
    uint32_t daemonId = 0;
    if (!sig || std::strcmp(sig, "u") != 0 ||
        sd_bus_message_read(m, "u", &daemonId) < 0 || daemonId == 0)
        return;
    for (auto& n : notes_)
        if (n.id == key) { n.daemonId = daemonId; break; }
}
