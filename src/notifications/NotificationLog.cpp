...
NotificationLog::~NotificationLog() {
    ...
    if (vtableSlot_) sd_bus_slot_unref(vtableSlot_);
...
bool NotificationLog::start() {
    ...
    static const sd_bus_vtable kVtable[] = {
        ...
        SD_BUS_METHOD("List", "", "a(xsssuy)", &NotificationLog::onList,
                      SD_BUS_VTABLE_UNPRIVILEGED),
        SD_BUS_VTABLE_END,
    };
    if (sd_bus_add_object_vtable(bus_, &vtableSlot_, notiflog::kObjectPath,
                                 notiflog::kInterface, kVtable, this) < 0)
        return false;
    int r = sd_bus_request_name(bus_, notiflog::kBusName, 0);
    if (r < 0) {
...
int NotificationLog::onList(sd_bus_message* m, void* userdata, sd_bus_error*) {
