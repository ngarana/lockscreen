...
std::string resolvePlaylistDir() {
    if (FILE* f = std::fopen("playlists/day.m3u", "r")) { std::fclose(f); return "playlists"; }
...
bool VideoPlayer::init(const std::string& playlistDir) {
    mpv_ = mpv_create();
    if (!mpv_) { std::fprintf(stderr, "qypr-lock: mpv_create failed\n"); return false; }
...
void VideoPlayer::stop() {
    ...
    if (mpv_) { mpv_destroy(mpv_); mpv_ = nullptr; }
    pixels_.clear();
    frameW_ = frameH_ = 0;
    haveContent_ = false;
}
...
void VideoPlayer::onWakeup(void* self) { static_cast<VideoPlayer*>(self)->handleWakeup(); }
...
void VideoPlayer::draw(cairo_t* cr, int outW, int outH) {
    ...
    cairo_surface_t* img = cairo_image_surface_create_for_data(
        pixels_.data(), CAIRO_FORMAT_RGB24, frameW_, frameH_, frameW_ * 4);
